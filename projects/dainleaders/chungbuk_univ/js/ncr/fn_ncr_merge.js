...
function fnSatisOnOff(satRschType, obj) {
	...
	if (satRschType == "std") {
		scaleQusArry = '프로그램(활동)에 대해 전반적으로 만족하십니까?';
		scaleQusArry += ',프로그램(활동)은 나의 역량 함양 및 능력 향상에 도움이 되었습니까?';
		scaleQusArry += ',프로그램(활동)의 내용과 주제는 적절하였습니까?';
		scaleQusArry += ',프로그램(활동)의 운영 기간 및 시간은 적절하였습니까?';
		scaleQusArry += ',프로그램(활동)과 관련된 시설/환경 등 여건은 적절하였습니까?';
		scaleQusArry += ',강사는 해당 프로그램 관련 전문성이 높았습니까?';
		scaleQusArry += ',강사는 강의 내용을 체계적으로 전달하였습니까?';
		scaleQusArry += ',강사는 적극적인 자세로 충실하게 강의하였습니까?';

		choiceQusArry = '프로그램 참여가 우리 대학 핵심역량 중 어느 역량 향상에 도움이 되었다고 생각하십니까?';
		choiceQusArry += ',프로그램(활동)에 참여하게 된 동기는 무엇입니까?';

		choiceAnsQusArry += '1^C-창의성';
		choiceAnsQusArry += ',1^H-휴머니즘';
		choiceAnsQusArry += ',1^A-능동성';
		choiceAnsQusArry += ',1^N-공동체';
		choiceAnsQusArry += ',1^G-글로벌';
		choiceAnsQusArry += ',1^E-전문성';

		choiceAnsQusArry += ',2^학과(교수)추천';
		choiceAnsQusArry += ',2^동기/선후배 추천';
		choiceAnsQusArry += ',2^흥미/관심분야';
		choiceAnsQusArry += ',2^취업준비';
		choiceAnsQusArry += ',2^학업에 도움';
	...
	if (viewType == 'MODIFY') {
		...
		if ('${programInfoMap.NPI_STAT_DB_CD}' == 'NCR_T05_P06'
			...
			if ('${sessionScope.loginVO.roleId}' == 'RLE0000024'
					|| '${sessionScope.loginVO.r9oleId}' == 'SUPERADMIN_01') {
 ...
 ****************************************
 */
function fnGoRecrList(_programGubun, _keyId, _bookProgramYn) {
	$('#detailForm #npiKeyId').val(_keyId);
	$('#detailForm #bookProgramYn').val(_bookProgramYn);
...
function fnSetProgramTerm(obj,typeFlag) {
	var chked_val = "";
	if (typeFlag == "order") {
		$(":checkbox[name=program_order]:checked").each(function(pi, po) {
			chked_val += po.value;
		});
		$("#addReqDeadLine").val(chked_val);
	} else if (typeFlag == "chk") {
		$(":checkbox[name=program_chk]:checked").each(function(pi, po) {
			chked_val += po.value;
		});
...
function fnCalMileageScore() {
	...
	if (npiCounselYn == "Y") {
		...
		if (isEmpty(npiPartyTime)) {
			...
			$("#abilitySumValTag").text(
					parseInt(npiPartyTime * calcNpiLevel * 100) + " 마일리지");
			$('input[name=npiAbilitySum]').val(npiPartyTime * calcNpiLevel * 100);
		}
	}
}
...
function fnApplyStdStateGraph(divNm, dataArry, categoriesArry) {
	var options = {
		...
		xaxis : {
			categories : [ '1학년', '2학년', '3학년', '4학년' ,'5학년' , '6학년'],
			labels : {
				formatter : function(val) {
