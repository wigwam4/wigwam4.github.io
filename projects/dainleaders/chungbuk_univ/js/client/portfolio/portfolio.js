...
function getDataDivHtml(pageNo, _url, searchKey, portFolioGbn, viewType, portType) {
	...
	//console.log("portFolioGbn = " + portFolioGbn);
...
representResume = function(obj,_resumeSeq, _stdNo, _resumeDiv){
	...
	_msg= _div+"로 설정하시겠습니까?";

	if( confirm(_msg) ){
		$.post(BASE_PATH+"/portfolio/w/n/updateRepresentResume.do"
			,{resumeSeq:_resumeSeq,stdNo:_stdNo,resumeDiv : _resumeDiv}
			,function(result){
				if( result == 'success'){
					if (_resumeDiv == "RESM") {
					...
					if (_resumeDiv == "RESM") {
						alert("대표 이력서로 설정하는데 실패하였습니다.\n잠시후 다시 시도해주세요");
					}
					else{
						alert("대표 자기소개서로 설정하는데 실패하였습니다.\n잠시후 다시 시도해주세요");
...
/* 포트폴리오 공유 URL 팝업*/
sharePortfolio = function (_stdNo, _portType){
