 ...
 */
function isEmpty(el) {
	return (el == null || el == "" || typeof el == "undefined" || el == "undefined") ? true : false;
}
 ...
 * @returns
 */
function fnSetCampusCdCombo(viewId, selValue, allValue, useAsync, basePathClient)  {
	var basePath = sessionStorage.getItem("contextRootPath");

	if(typeof basePath == "undefined" && basePath(allValue) || basePath == null || basePath == 'null') {
		basePath = basePathClient;
	}
	var _url = basePath+"/cmm/fms/getCampusCodeListAjax.do";
	var async = isNullObject(useAsync) ? true : useAsync;
	var targetView = $("#"+viewId);
	if(typeof targetView == "undefined") {return;}
	...
	jQuery.ajax({
		...
		success: function(r) {
			if (r.beanlist.length > 0) {
				for(var idx=0; idx<r.beanlist.length; idx++) {
					var obj = r.beanlist[idx];
					if(typeof selValue != "undefined" && selValue == obj.CAMPUS_CD) {
						targetView.append("<option value='"+obj.CAMPUS_CD+"' selected>"+obj.CAMPUS_CD_NM+"</option>");
					} else {
						targetView.append("<option value='"+obj.CAMPUS_CD+"'>"+obj.CAMPUS_CD_NM+"</option>");
					}
				}
			}
		},
		error: function(r) {
			console.log("캠퍼스 코드 조회중 오류가 발생하였습니다.");
		}
	}) ;
}
 ...
 * @returns
 */
function fnSetShregStCombo(viewId, selValue, allValue, useAsync, basePathClient)  {
	var basePath = sessionStorage.getItem("contextRootPath");
	
	if(typeof basePath == "undefined" && basePath(allValue) || basePath == null || basePath == 'null') {
		basePath = basePathClient;
	}
	var _url = basePath+"/cmm/fms/getShregStListAjax.do";
	var async = isNullObject(useAsync) ? true : useAsync;
	var targetView = $("#"+viewId);
	if(typeof targetView == "undefined") {return;}
	...
	jQuery.ajax({
		...
		success: function(r) {
			if (r.beanlist.length > 0) {
				for(var idx=0; idx<r.beanlist.length; idx++) {
					var obj = r.beanlist[idx];
					if(typeof selValue != "undefined" && selValue == obj.SHREG_ST) {
						targetView.append("<option value='"+obj.SHREG_ST+"' selected>"+obj.SHREG_ST_NM+"</option>");
					} else {
						targetView.append("<option value='"+obj.SHREG_ST+"'>"+obj.SHREG_ST_NM+"</option>");
					}
				}
			}
		},
		error: function(r) {
			console.log("학적 코드 조회중 오류가 발생하였습니다.");
		}
	}) ;
}
 ...
 * @returns
 */
function fnSetCampusCdCheckBox(viewId, chkdValue, titleValue, valueName, userGubun,useAsync)  {
	var basePath = sessionStorage.getItem("contextRootPath");
	var _url = basePath+"/cmm/fms/getCampusCodeListAjax.do";
	...
	jQuery.ajax({
		...
		success: function(r) {
			if (r.beanlist.length > 0) {
				for(var idx=0; idx<r.beanlist.length; idx++) {
					var obj = r.beanlist[idx];
					var checked = chkdValue.indexOf(obj.CAMPUS_CD) != -1 ? 'checked' : '';

					console.log("chkdValue = "+chkdValue+"\n"+"CAMPUS_CD = "+obj.CAMPUS_CD+"\n checked ="+checked+"\n\n");

					if (userGubun == "client") {

						targetView.append(
								'<p class="check_row">'
								+'<input name="'+valueName+'" class="check '+viewId+'Check hidden '+viewId+'All" id="'+viewId+idx+'" type="checkbox" value="'+obj.CAMPUS_CD+'" title="'+titleValue+'" '+checked+'>'
								+'<label for="'+viewId+idx+'">'+obj.CAMPUS_CD_NM+'</label>'
								+'</p>'
						);
					}else{
						targetView.append(
								'<p class="ad_input_row">'
								/*기타 input박스 있을경우,*/
								//+'<input name="'+valueName+'" class="check '+viewId+'Check '+viewId+'All" id="'+viewId+idx+'" type="checkbox" value="'+obj.CAMPUS_CD+'" title="'+titleValue+'" onclick="fnCreateEtcInputBox('+"'"+obj.CAMPUS_CD+"'"+')" onchange="fn_checkbox('+"'"+viewId+'All'+"'"+", '"+viewId+'All'+"'"+');" '+checked+'>'

								/*기타 input박스 없을경우,*/
								+'<input name="'+valueName+'" class="check '+viewId+'Check '+viewId+'All" id="'+viewId+idx+'" type="checkbox" value="'+obj.CAMPUS_CD+'" title="'+titleValue+'"  onchange="fn_checkbox('+"'"+viewId+'All'+"'"+", '"+viewId+'All'+"'"+');" '+checked+'>'
								+'<label class="check_label" for="'+viewId+idx+'">'+obj.CAMPUS_CD_NM+'</label>'
								+'</p>'
						);
					}
				}
			} else {
				alert("캠퍼스코드 조회중 오류가 발생하였습니다.");
			}
		},
		error: function(r) {
			console.log("캠퍼스코드 조회중 오류가 발생하였습니다.");
		}
	}) ;
...
****************************************
**/
function fnExcelFailPop(_totalCnt ,_failCnt, _successCnt, _abililtyYn) {
	var basePath = sessionStorage.getItem("contextRootPath");
...
function fnOpenNcrCertiHisPopUp(stdNo,type){
	var basePath = sessionStorage.getItem("contextRootPath");
	var url = basePath +"/cmm/fms/getStdCertiTimeHisPopUp.do?stdNo="+ stdNo +"&type="+type;
	window.open(url,"stdCertiHisForm","toolbar=no,directories=no,scrollbars=yes,resizable=no,status=no,menubar=no,width=1200, height=830, left=100");
}