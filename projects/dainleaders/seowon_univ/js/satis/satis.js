 ...
 * 만족도 조사 기능 시작
 */
function fnSatisOnOffAjax(satRschType, obj, scaleQusArry, openQusArry, choiceQusArry, choiceAnsQusArry, qusType) {
	...
	if(_val == 'std_1' || _val == 'std_2' || _val == 'std_3') {
		
		$("#"+qusType+"SatRschTr").remove();
		chkHtmlTr = $("#"+qusType+"SatRschTr").html();
...
//		var _obj = $("#"+_id).parent("p").parent("td").parent("tr").parent("tbody ");
		var _obj = $("#"+_id).parent("td").parent("tr").parent("tbody ");
		// 만족도 폼 ajax
		$.ajax({
			url : _url
			, type : "POST"
	...
	// 만족도 미사용 선택
	}else if(satRschType == 'pre' && _val == 'Y'){
		if(typeof chkHtmlTr != "undefined") {
			return false;
		}
		// 만족도 폼 추가할 위치
		...
		if(qusType == 'std'){
			if( confirm("입력하신 문항내용이 모두 지워집니다. 변경하시겠습니까?") ) {
				$("#"+qusType+"SatRschTr").remove();
			}
		}else{
			// 만족도 테이블 삭제되어애함
