...
function addQus(type) {
	...
	if( type == "peer" ) {  
		...
		$tableTr.find("tbody").append("<tr>"
				+"<td class='qusNum'></td>"
				+"<td class='td_input'>"
					+"<input type='text' class='form-control input-sm' name='sqmCont3' onkeyup='fnKeyUpCommaChk(this);' title='질문 내용'>"
				+"</td>"
				+"<td class='td_input'>"
...

{
	...
	if( type == "std" ) {
		...
		if(_val == "SCALE") {
			...
			$tableTd.prev().append("<input type='text' class='form-control input-sm' name='sqmCont1' onkeyup='fnKeyUpCommaChk(this);' title='질문 내용'>");
...
//							+"<span class='stit_bullet'></span>자유형 객관식 답변 작성"
						...
						+"<div class='ad_btn_area1 mg_t10'>"
							...
							+"<p class='ad_input_row'>"
								+"<input name='sqmMultiYnChk1' class='check mg_l5' id='sqmMultiYnChk"+_row+"' type='checkbox' onclick='sqmMultiYnClick(this, \"std\");'>"
	...
	if( type == "pre" ) {
		...
		if(_val == "SCALE") {
			...
			$tableTd.prev().empty();
			// 이전요소(질문정보가 들어 있는 td)질문 내용 넣기
			$tableTd.prev().append("<input type='text' class='form-control input-sm' name='sqmCont2' onkeyup='fnKeyUpCommaChk(this);' title='질문 내용'>");
